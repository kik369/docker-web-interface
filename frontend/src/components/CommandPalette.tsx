...
const CommandPalette: React.FC<CommandPaletteProps> = React.memo(({
    ...
    const getIconComponent = (iconName: string) => {
        switch (iconName) {
            case 'container':
                return (
                    <svg className="w-5 h-5 text-blue-300" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11 17a1 1 0 001.447.894l4-2A1 1 0 0017 15V9.236a1 1 0 00-1.447-.894l-4 2a1 1 0 00-.553.894V17zM15.211 6.276a1 1 0 000-1.788l-4.764-2.382a1 1 0 00-.894 0L4.789 4.488a1 1 0 000 1.788l4.764 2.382a1 1 0 00.894 0l4.764-2.382zM4.447 8.342A1 1 0 003 9.236V15a1 1 0 00.553.894l4 2A1 1 0 009 17v-5.764a1 1 0 00-.553-.894l-4-2z" />
                    </svg>
                );
            case 'image':
                return (
                    <svg className="w-5 h-5 text-purple-300" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            ...
            case 'logs':
                return (
                    <HiOutlineDocumentText className="w-5 h-5 text-blue-300" />
                );
            default:
                return null;
        }
    ...
    const StatusIndicator = ({ status }: { status?: 'running' | 'stopped' | 'none' }) => {
        ...
        return (
            <span className={`w-2 h-2 rounded-full mr-2 ${status === 'running' ? 'bg-green-400 animate-soft-pulse' : 'bg-red-500'
                }`}></span>
        );
    };
                                                ...
                                                >
                                                    <div className="flex justify-between items-center">
                                                        <div className="flex items-center">
                                                            ...
                                                            <span className={`${command.category === 'Containers' || command.category === 'Images' ? 'font-mono' : 'font-medium'}`}>
                                                                {command.category === 'Containers' || command.category === 'Images'
                                                                    ? command.name.replace('Container: ', 'container: ').replace('Image: ', 'image: ')
                                                                    : command.name}
                                                            </span>
                                                        </div>
                                                        {command.shortcut && (
